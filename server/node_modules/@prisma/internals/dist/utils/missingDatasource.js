"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var missingDatasource_exports = {};
__export(missingDatasource_exports, {
  missingDatasource: () => missingDatasource
});
module.exports = __toCommonJS(missingDatasource_exports);
var import_colors = require("kleur/colors");
var import_highlight = require("../highlight/highlight");
var import_link = require("./link");
const missingDatasource = `
You don't have any ${(0, import_colors.bold)("datasource")} defined in your ${(0, import_colors.bold)("schema.prisma")}.
You can define a datasource like this:

${(0, import_colors.bold)(
  (0, import_highlight.highlightDatamodel)(`datasource db {
  provider = "postgresql"
  url      = env("DB_URL")
}`)
)}

More information in our documentation:
${(0, import_link.link)("https://pris.ly/d/prisma-schema")}
`;
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  missingDatasource
});
